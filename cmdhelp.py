...
class CmdHelp:
    ...
    def get_result(self):
        ...
        result = f"**馃摋 File :** `{self.FILE}`\n"
        if self.WARNING == "" and self.INFO == "":
            result += f"**猬囷笍 Official:** {'鉁�' if self.IS_OFFICIAL else '鉂�'}\n\n"
        else:
            result += f"**猬囷笍 Official:** {'鉁�' if self.IS_OFFICIAL else '鉂�'}\n"

            if self.INFO == "":
                if not self.WARNING == "":
                    result += f"**鈿狅笍 Warning :** {self.WARNING}\n\n"
            else:
                if not self.WARNING == "":
                    result += f"**鈿狅笍 Warning :** {self.WARNING}\n"
                result += f"**鈩癸笍 Info:** {self.INFO}\n\n"

        for command in self.COMMANDS:
            command = self.COMMANDS[command]
            if command["params"] == None:
                result += (
                    f"**馃洜 Command :** `{COMMAND_HAND_LER[:1]}{command['command']}`\n"
                )
            else:
                result += f"**馃洜 Command :** `{COMMAND_HAND_LER[:1]}{command['command']} {command['params']}`\n"

            if command["example"] == None:
                result += f"**馃挰 Details :** `{command['usage']}`\n\n"
            else:
                result += f"**馃挰 Details :** `{command['usage']}`\n"
                result += f"**鈱笍 For Example :** `{COMMAND_HAND_LER[:1]}{command['example']}`\n\n"
        return result
