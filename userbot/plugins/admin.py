...
async def promote(promt):
    ...
    try:
        ...
        await hellevent.edit("`SKYLORD PROMOTING USER FOR 2 WEEKS..ADD MORE MEMBERS TO INCREASE UR CONTRACT...馃拑馃暫`")
...
async def demote(dmod):
    ...
    try:
        ...
        await hellevent.edit(NO_PERM)
        return
    await hellevent.edit("`SKYLORD REMOVED USER FROM ADMIN....馃毝`")
    if BOTLOG:
...
async def ban(bon):
    ...
    if reason:
        ...
        await skyevent.edit(f"{str(user.id)} is banned馃槒 !!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    try:
        ...
        await skyevent.edit("```Unbanned Successfully. Granting another chance馃毝.```")
        if BOTLOG:
...
async def startmute(event):
    if event.is_private:
        ...
        if is_muted(userid, chat_id):
            return await event.edit("This user is already muted馃ぃ馃ぃ馃ぃ")
        try:
            mute(userid, chat_id)
        except Exception as e:
        ...
        if is_muted(user.id, event.chat_id):
            return await edit_or_reply(
                event, "This user is already muted in this chat馃槒馃毝"
            )
        try:
            admin = chat.admin_rights
            creator = chat.creator
            if not admin and not creator:
                await edit_or_reply(
                    event, "`NOOB ADMINS AND OWNER DONT GAVE US RIGHTS...` 嗖ワ箯嗖�  "
                )
                return
            ...
            try:
                if result.participant.banned_rights.send_messages:
                    return await edit_or_reply(
                        event,
                        "This user is already muted in this chat 馃槒馃毝",
                    )
            except:
                pass
            ...
            if "admin_rights" in vars(chat) and vars(chat)["admin_rights"] is not None:
                if chat.admin_rights.delete_messages is not True:
                    return await edit_or_reply(
                        event,
                        "`NOOB ADMINS AND OWNER DONT GAVE US RIGHTS(DELETE MESSAGES). 嗖ワ箯嗖",
                    )
            elif "creator" not in vars(chat):
                return await edit_or_reply(
                    event, "`NOOB ADMINS AND OWNER DONT GAVE US RIGHTS(DELETE MESSAGES).` 嗖ワ箯嗖�  "
                )
            try:
        ...
        try:
            if is_muted(user.id, event.chat_id):
                ...
                try:
                    ...
                    return await edit_or_reply(
                        event,
                        "This user can already speak freely in this chat馃槳",
                    )
        except Exception as e:
            return await edit_or_reply(event, f"**Error : **`{str(e)}`")
