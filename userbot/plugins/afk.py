    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await borg.send_message(
                event.chat_id,
                f"__**I'm going OFFLINE馃毝**__ \n鈿滐笍 Because `{reason}`",
                file=skypic,
            )
        else:
            await borg.send_message(
                event.chat_id, f"**I am Going OFFLINE!**馃毝", file=skypic
            )
...
CMD_HELP.update(
    {
        "afk": "**Syntax :** \n鉁� .afk <reason> \n鉁� .afk <reply to media> \n鉁� .afk <reason> <reply to media>\n"
